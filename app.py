...
html_temp = """
		    ...
		    <h4 style="color:white;text-align:center;">Insurance Team </h4>
		    </div>
            """
...
def main(): #Loading tampilannya aja
    stc.html(html_temp)
...
def run_ml_app():
    ...
    if button:
        ...
        if result == 'Eligible':
            st.success(f'You are {result} for the loan') #Warna ijo
        else:
            st.warning(f'You are {result} for the loan') #Warna kuning, kalo merah danger


def predict(gender,married,dependent,education, self_employed, applicant_income,coApplicant_income,
                         loan_amount, loan_amount_term, credit_history,property_area):
    #processing user input (Kategorikal value)
